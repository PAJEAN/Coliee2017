...
sentences = []
stoplist = set('for a of the and to in after before without an its by'.split())
for id_article in xml_parser.articles:
	texts = [word for word in xml_parser.articles[id_article].lower().split() if word not in stoplist]
	sentences.append(texts)

model = Word2Vec(sentences)
print(model.similarity('owner', 'possessor'))
print(model.most_similar(positive=["person", "manager"]))
print(model.most_similar(positive=["manager"]))
