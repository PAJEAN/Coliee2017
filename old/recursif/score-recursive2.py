...
def loadBestKTerrierResults(path_terrier_results, k_limit, docToPart, queriesToParts):
            ...
            with open(f_path) as terrier_result_file:
                for line in terrier_result_file:
                    ...
                    if len(spl) == 6:
                        ...
                        if id_query in queriesToParts:
                            if docToPart[data_doc[0]] in queriesToParts[id_query]:
...
# path to the index storing the part associated to each query
#queriesPartsPath = "tmp/queriesToChapter.tsv"#"tmp/queriesToParts.tsv"
queriesPartsPath = "tmp/queryTopart_predicted.tsv"
#queriesPartsPath = "tmp/queriesToPart_groundtruth.tsv"
#queriesPartsPath = "tmp/queriesToSection.tsv"
# path of the index storing the part associated to each article
#articleToPartPath = "tmp/articleToChapter.tsv" #"tmp/articleToPart.tsv"
articleToPartPath = "tmp/articleToPart.tsv"
#articleToPartPath = "tmp/articleToSection.tsv"
...
docToArticle = {}
# Index providing a query to parts mapping
# dict : id_query -> list of parts ids
queriesToParts = {}
# Index providing ans article to part mapping
# dict : id_doc -> part id
docToPart = {}
...
# Loading correspondences between indexed parts and queries
with open(queriesPartsPath) as query2partFile:
    for l in query2partFile:        
        token = l.strip().split("\t")
        queryId = token[0]
        partId = token[1]
        if queryId not in queriesToParts:
            queriesToParts[queryId] = [partId]
        else:
            queriesToParts[queryId].append(partId)
#print {q:queriesToParts[q] for q in queriesToParts if len(queriesToParts[q]) > 1}

# Loading correspondences between indexed parts and articles
with open(articleToPartPath) as article2partFile:
    for l in article2partFile:
        token = l.strip().split("\t")
        articleId = token[0]
        partId = token[1]
        for docId in [id_doc for id_doc in docToArticle if articleId in docToArticle[id_doc]]:
            docToPart[docId] = partId            
#print docToPart
...
terrier_results_bestk = loadBestKTerrierResults(path_terrier_results, NB_RESULTS_DEEP_ANALYSIS, docToPart, queriesToParts)
...
for query_id in terrier_results_bestk:
    ...
    if articles_first_result_doc is not None:
        if (THRESHOLD_TERRIER_SCORE == -1 or doc_score > THRESHOLD_TERRIER_SCORE):
            print list(articles_first_result_doc)
